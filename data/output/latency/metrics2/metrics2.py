...
def logDump(query, approach, meanList, stdList):
    with open("metrics2.info.{}.log".format(round(startTime)), "a") as w:
        w.write("{}\n".format(query))
        w.write("{}\n".format(approach))
        ...
        for approach in approaches:
            ...
            for l in glob.glob("{}/{}/*.log".format(query, approach)):
                ...
                if plotLatency:
                    ...
                    plt.figure()
                    plt.plot(range(validarray.size), validarray)
                    print("*** Save fig ***")
...
def writeResults(results):
    with open("metrics2.result.{}.txt".format(startTime), "w") as w:
        # Write mean

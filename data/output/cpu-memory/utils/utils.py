...
def shapeData(queries, approaches, results, rawData, maxCount):
    ...
    for idx in range(maxCount):
        ...
        for query in queries:
            ...
            for approach in approaches:
                cpu_value = results[query][approach]["CPU"]["value"][idx] if idx < len(results[query][approach]["CPU"]["value"]) else np.nan
                cpu_std = results[query][approach]["CPU"]["std"][idx] if idx < len(results[query][approach]["CPU"]["std"]) else np.nan
                cpu_raw_list = rawData[query][approach]["CPU"][idx] if idx < len(rawData[query][approach]["CPU"]) else []
                memory_value = results[query][approach]["Memory"]["value"][idx] if idx < len(results[query][approach]["Memory"]["value"]) else np.nan
                memory_std = results[query][approach]["Memory"]["std"][idx] if idx < len(results[query][approach]["Memory"]["std"]) else np.nan
...
def calcResults(queries, approaches, filterRate, plotTrends, startTime, size):
    ...
    for query in queries:
        for approach in approaches:
            ...
            if len(files) == 0:
                results[query][approach]["CPU"]["value"].append(np.nan)
                results[query][approach]["CPU"]["std"].append(np.nan)
                results[query][approach]["Memory"]["value"].append(np.nan)
                results[query][approach]["Memory"]["std"].append(np.nan)
                rawData[query][approach]["CPU"].append([])
                rawData[query][approach]["Memory"].append([])
                maxCount = 1
            elif len(files) >= 1:
                for file in files:
                    ...
                    means = df.mean()
                    cpu_means = means["CPU"]
                    mem_means = means["Memory"]
                    stds = df.std(ddof=0)
                    cpu_stds = stds["CPU"]
                    mem_stds = stds["Memory"]

                    results[query][approach]["CPU"]["value"].append(cpu_means)
                    results[query][approach]["CPU"]["std"].append(cpu_stds)
                    results[query][approach]["Memory"]["value"].append(mem_means)
                    results[query][approach]["Memory"]["std"].append(mem_stds)
                    maxCount = len(results[query][approach]["CPU"]["value"])

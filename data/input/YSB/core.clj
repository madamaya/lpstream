...
;; Original code generates data in real time.
;; Generating data between 1 second in the simulation takes one second.
;; It take much time to obtain the massive size of data.
;; Therefore, I modified a code to generate input data at a high rate.
(ns setup.core
    ...
    (with-open [p (producer {"bootstrap.servers" kafka-hosts}
      ...
      (doseq [t times]
          (send p (record "ad-events"
                          (.getBytes (make-kafka-event-at (/ t 1000000) with-skew? ads user-ids page-ids))))))))

...
public class AuctionDataParserNex extends RichMapFunction<KafkaInputString, NexmarkAuctionTuple> {
    ...
    public NexmarkAuctionTuple map(KafkaInputString input) throws Exception {
        ...
        if (eventType == 1) {
            ...
            NexmarkAuctionTuple tuple = new NexmarkAuctionTuple(eventType, auctionId, itemName, desc, initBid, reserve, dateTime, expires, seller, category, extra, input.getKafkaAppandTime(), input.getStimulus());

...
public class AuctionDataParserNexGL implements MapFunction<StringGL, NexmarkAuctionTupleGL> {
    ...
    public NexmarkAuctionTupleGL map(StringGL input) throws Exception {
        ...
        if (eventType == 1) {
            ...
            NexmarkAuctionTupleGL out = new NexmarkAuctionTupleGL(eventType, auctionId, itemName, desc, initBid, reserve, dateTime, expires, seller, category, extra, input.getKafkaAppandTime(), input.getStimulus());
